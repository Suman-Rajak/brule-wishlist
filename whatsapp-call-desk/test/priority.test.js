...
test('read your question and did not reply → call first', () => {
  const c = chat([msg(3 * DAY, 'them', 'How much for 1kg?'), msg(2 * DAY, 'me', '₹2,400. Want me to send it?')]);
  const r = run(c, ai(c, { interest: 70 }));
  assert.equal(r.tier, 'first');
  assert.equal(r.leftOnSeen, true);
  assert.match(r.reason, /Read your message 2 days ago/);
...
test('logged calls: interested → done, no answer stays with a badge, 3 no-answers → later', () => {
  const c = chat([msg(3 * DAY, 'them', 'Price?'), msg(2 * DAY, 'me', '₹650 — want it?')]);
...
test('your manual move wins, and a stale AI read falls back to the reply heuristic', () => {
  const c = chat([msg(3 * DAY, 'them', 'Hi'), msg(2 * DAY, 'me', 'Hello! How can I help?')]);
